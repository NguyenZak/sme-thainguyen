    ...
    <form onSubmit={handleSave} className="space-y-6 max-w-4xl">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-4 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1">
              Chi Phí Ăn Trưa Đại Biểu Phát Sinh <span className="text-amber-600 font-bold">*(VNĐ / 1 bữa / người)*</span>
            </label>
            <input
              type="number"
              value={fee.extraDelegateLunchPriceVND !== undefined ? fee.extraDelegateLunchPriceVND : 100000}
              placeholder="100000"
              onChange={(e) => setFee({ ...fee, extraDelegateLunchPriceVND: Number(e.target.value) })}
              className="w-full bg-white border border-slate-300 rounded-xl px-3.5 py-2.5 text-xs text-slate-900 focus:border-slate-900 focus:ring-1 focus:ring-slate-900 focus:outline-none font-bold"
            />
            <p className="text-[11px] text-slate-500 mt-1">
              Hiển thị: <strong className="text-emerald-700">{(fee.extraDelegateLunchPriceVND || 100000).toLocaleString("vi-VN")} VNĐ</strong> / bữa (Miễn phí 2 bữa chính cho ĐB trong gói)
            </p>
          </div>
