  ...
  return (
    <div className="space-y-6 max-w-6xl">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-4">
        <div>
          <h2 className="text-xl font-bold text-white">Quản Lý Danh Sách Đăng Ký Tham Dự</h2>
          <p className="text-xs text-slate-400">Xem, tìm kiếm, xác nhận trạng thái và xuất danh sách khách hàng tham dự sang Excel/CSV.</p>
        </div>

        <div className="flex items-center gap-3">
          <button
            onClick={fetchRegistrations}
            className="inline-flex items-center gap-1.5 px-3 py-2 bg-slate-900 hover:bg-slate-800 border border-slate-800 rounded-xl text-xs font-semibold text-slate-300 transition-colors"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-slate-950 rounded-xl text-xs font-bold transition-all shadow-md shadow-emerald-500/20 disabled:opacity-50"
          >
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3">
        ...
        <select
          ...
          className="bg-slate-900 border border-slate-800 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none w-full sm:w-auto font-semibold"
        >
                ...
                {filteredList.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-800/40 transition-colors">
                    <td className="px-4 py-3.5 font-bold text-white">
                      {r.full_name}
                      {r.notes && <div className="text-[11px] font-normal text-slate-400 mt-0.5 italic">Ghi chú: {r.notes}</div>}
                    </td>
                    <td className="px-4 py-3.5">
                      <div className="font-semibold text-slate-200">{r.company_name}</div>
                      <div className="text-[11px] text-slate-400">{r.position}</div>
                    </td>
                    <td className="px-4 py-3.5">
                      <div className="font-mono text-emerald-400">{r.phone}</div>
